<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="com.scale.product.model.vo.Product, java.util.ArrayList, com.scale.style.model.vo.*" %>
<%
...
<body>
    ...
    <div class="outer" align="center">
        <br>
        <h2 style="font-size: 30px;"><b>스타일 변경</b></h2>
        <br>

        <form action="<%= contextPath %>/change.st" method="post" enctype="multipart/form-data">
			<input type="hidden" name="styleNo" value="<%= st.getStyleNo() %>">
            ...
            <table>
                <tr>
                    <td id="style-box">
                        <div style="text-align: center;">
                        	<img id='styleImg' src="<%= contextPath %>/<%= ilist.get(0).getFilePath() %><%= ilist.get(0).getChangeName() %>">
                            <span style="font-size: 30px; color: lightgray;" style="display: none">대표 이미지</span>
                        </div>
                    </td>
                    <td id="thumbnail-td">
                        <div id="input-box">
                ...
                <tr>
                    <td colspan="2" id="pd-thumbnail-td">
                    	...
                    	<ul class='pd-thumbnail'>
                    		<li>
                    			<input type='hidden' name='pCode' value='<%= plist.get(p).getProductCode() %>'>
                    			<img src='<%= plist.get(p).getProductImgM() %>'>
                    			<div class='delete-pd'>❌</div>
                    		</li>
                    		<li class='li-brand'><%= plist.get(p).getBrandName() %></li>
                    		<li class='li-ko'><%= plist.get(p).getProductNameKo() %></li>
                    		<li class='li-eng'><%= plist.get(p).getProductNameEng() %></li>
                    	</ul>
                    	<% } %>
                    </td>
                </tr>
                <tr>
                    <td colspan="2" id="content-td">
                        ...
                        <textarea name="content" id="content" maxlength="1000" style="resize: none; border: none;" placeholder="아이템과 스타일을 자랑해보세요..." required><%= st.getContent() %></textarea>
                        <% } %>
                    </td>
                </tr>
                <tr>
                        ...
                        <div id="tag-box">
                            ...
                            <ul id="tag-list">
                            	<% if(st.getHashtag() != null) { %>
	                            	...
	                            	<input type='hidden' name='tag' value='<%= tagArr[t] %>'>
	                            	<li class='tag-li'><%= tagArr[t] %></li><div class='tag-delete'>❌</div>
	                            	<% } %>
                            	<% } %>
                            </ul>
                            <script>
            ...
            <div>
                <button type="submit" class="btn btn-secondary" onclick="check();">수정하기</button>
